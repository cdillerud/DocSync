...
"@
    ...
    elseif ($Content -match '<Page>\s*') {
        $Updated = [regex]::Replace(
            $Content,
            '<Page>\s*',
            '<Page>' + $LineEnding + $FooterXml + $LineEnding,
            1)
    }
    else {
...
$BackupRoot = Join-Path -Path $RepoRoot -ChildPath ".gpi-backups\footer-address-only-027007-v2-$Timestamp"
...
foreach ($LayoutFile in $LayoutFiles) {
    ...
    $UpdatedLayouts[$LayoutFile] = Set-ReportPageFooter `
...
try {
    ...
    Write-Host " GPI footer address-only layout pass 0.27.0.7 v2" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
