...
function Add-ReceivingReportFields {
    ...
    if ($Content -notmatch 'GPIBuildGamerContactNames\(PurchaseHeader\);') {
        ...
        if ($ContactLineMatches.Count -ne 1) {
            throw "Could not find a safe insertion point for Receiving Notice Gamer Contacts in $Label. Found $($ContactLineMatches.Count) PurchaserName contact-line blocks."
        }
...
$BackupRoot = Join-Path -Path $RepoRoot -ChildPath ".gpi-backups\rhonda-document-header-layout-027009-v3-$Timestamp"
...
try {
    ...
    Write-Host " GPI Rhonda document header layout pass 0.27.0.9 v3" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
