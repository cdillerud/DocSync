...
'@
        ...
        $SalespersonPattern = '(?s)Clear\(SalespersonName\);\s*if Salesperson\.Get\("Salesperson Code"\) then\s*SalespersonName := Salesperson\.Name;'
        $SalespersonMatches = [regex]::Matches($Content, $SalespersonPattern)

        if ($SalespersonMatches.Count -eq 1) {
            $Match = $SalespersonMatches[0]
            $Replacement = $Match.Value + $InsideSalesAssignment
            $Content = $Content.Substring(0, $Match.Index) + $Replacement + $Content.Substring($Match.Index + $Match.Length)
        }
        else {
            $CurrencyPattern = '(?m)^(\s*)CurrencyCode := "Currency Code";'
            $CurrencyMatches = [regex]::Matches($Content, $CurrencyPattern)

            if ($CurrencyMatches.Count -ne 1) {
                throw "Could not find a safe insertion point for Blanket Sales Order Gamer Contacts in $Label. Found $($SalespersonMatches.Count) salesperson blocks and $($CurrencyMatches.Count) currency blocks."
            }

            $Match = $CurrencyMatches[0]
            $Replacement = $InsideSalesAssignment + "`r`n" + $Match.Value
            $Content = $Content.Substring(0, $Match.Index) + $Replacement + $Content.Substring($Match.Index + $Match.Length)
        }
    }
...
function Add-PurchaseReportFields {
    ...
    if ($Content -notmatch 'GPIBuildGamerContactNames\(PurchaseHeader\);') {
        $ResolvePattern = '(?m)^(\s*)ResolvePurchaserName\(PurchaseHeader\);\s*$'
        $ResolveMatches = [regex]::Matches($Content, $ResolvePattern)

        if ($ResolveMatches.Count -eq 1) {
            $Match = $ResolveMatches[0]
            $Indent = $Match.Groups[1].Value
            $Replacement =
                $Match.Value + "`r`n" +
                $Indent + "GPIBuildGamerContactNames(PurchaseHeader);`r`n" +
                $Indent + "FOBText := GPIGetFieldValue(PurchaseHeader, 'fob');"
            ...
            if ($CurrencyMatches.Count -ne 1) {
                throw "Could not find a safe insertion point for Purchase Order Gamer Contacts in $Label. Found $($ResolveMatches.Count) ResolvePurchaserName blocks and $($CurrencyMatches.Count) CurrencyCode blocks."
            }
            ...
            $Insertion =
                $Indent + "GPIBuildGamerContactNames(PurchaseHeader);`r`n" +
                $Indent + "FOBText := GPIGetFieldValue(PurchaseHeader, 'fob');`r`n`r`n"

            $Content = $Content.Substring(0, $Match.Index) + $Insertion + $Content.Substring($Match.Index)
        }
    }
...
'@
    ...
    if ($Content -notmatch 'GamerContactName1: Text\[100\];') {
        $PurchaserVarPattern = '(?m)^(\s*)PurchaserName: Text\[100\];\s*$'
        $PurchaserVarMatches = [regex]::Matches($Content, $PurchaserVarPattern)

        if ($PurchaserVarMatches.Count -ne 1) {
            throw "Could not find exactly one PurchaserName variable in $Label. Found $($PurchaserVarMatches.Count)."
        }

        $Match = $PurchaserVarMatches[0]
        $Indent = $Match.Groups[1].Value
        $Replacement =
            $Match.Value + "`r`n" +
            $Indent + "FOBText: Text[100];`r`n" +
            $Indent + "GamerContactName1: Text[100];`r`n" +
            $Indent + "GamerContactName2: Text[100];"
...
function Add-ReceivingReportFields {
    ...
    if ($Content -notmatch 'column\(GamerContactName1; GamerContactName1\)') {
        $PurchaserColumnPattern = '(?s)(\s*column\(PurchaserName; PurchaserName\) \{ \})'
        $PurchaserColumnMatches = [regex]::Matches($Content, $PurchaserColumnPattern)

        if ($PurchaserColumnMatches.Count -ne 1) {
            throw "Could not find exactly one PurchaserName dataset column in $Label. Found $($PurchaserColumnMatches.Count)."
        }

        $Match = $PurchaserColumnMatches[0]
        $Replacement =
            $Match.Value + "`r`n" +
            '            column(GamerContactName1; GamerContactName1) { }' + "`r`n" +
            '            column(GamerContactName2; GamerContactName2) { }'
    ...
    if ($Content -notmatch 'GPIBuildGamerContactNames\(PurchaseHeader\);') {
        $ContactLinePattern = "(?m)^(\s*)if PurchaserName <> '''' then"
        $ContactLineMatches = [regex]::Matches($Content, $ContactLinePattern)

        if ($ContactLineMatches.Count -eq 1) {
            $Match = $ContactLineMatches[0]
            $Indent = $Match.Groups[1].Value
            $Insertion = $Indent + "GPIBuildGamerContactNames(PurchaseHeader);`r`n`r`n"

            $Content = $Content.Substring(0, $Match.Index) + $Insertion + $Content.Substring($Match.Index)
        }
        else {
            $ShipmentPattern = '(?m)^(\s*)Clear\(ShipmentMethodDescription\);'
            $ShipmentMatches = [regex]::Matches($Content, $ShipmentPattern)

            if ($ShipmentMatches.Count -ne 1) {
                throw "Could not find a safe insertion point for Receiving Notice Gamer Contacts in $Label. Found $($ContactLineMatches.Count) PurchaserName contact-line blocks and $($ShipmentMatches.Count) shipment blocks."
            }

            $Match = $ShipmentMatches[0]
            $Indent = $Match.Groups[1].Value
            $Insertion = $Indent + "GPIBuildGamerContactNames(PurchaseHeader);`r`n`r`n"
...
'@
        ...
        $ObjectVarMatches = [regex]::Matches($Content, '(?m)^\s{4}var\s*$')
        if ($ObjectVarMatches.Count -lt 1) {
            throw "Could not find the object-level var section in $Label."
        }

        $InsertAt = $ObjectVarMatches[$ObjectVarMatches.Count - 1].Index
        $Content = $Content.Substring(0, $InsertAt) + $Procedures + $Content.Substring($InsertAt)
    }
...
$BackupRoot = Join-Path -Path $RepoRoot -ChildPath ".gpi-backups\rhonda-document-header-layout-027009-v5-$Timestamp"
...
try {
    ...
    Write-Host " GPI Rhonda document header layout pass 0.27.0.9 v5" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
