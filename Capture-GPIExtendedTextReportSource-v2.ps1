...
$FoldersToCapture = @(
    ...
    (Join-Path $ProductionRoot "src\layout"),
...
foreach ($Folder in $FoldersToCapture) {
    if (Test-Path -LiteralPath $Folder) {
        Get-ChildItem -LiteralPath $Folder -Recurse -File |
            Where-Object {
                $_.Extension -in @(".al", ".rdl", ".rdlc", ".json", ".docx")
            } |
            ForEach-Object {
                $FullName = $_.FullName
                $ShouldInclude = $false
                ...
                if (-not $ShouldInclude -and $_.Extension -in @(".al", ".json")) {
...
foreach ($File in $Files) {
    ...
    if ([System.IO.Path]::GetExtension($File) -ieq ".docx") {
        $Lines.Add("[Binary DOCX layout captured by file path/hash only]")
    }
    else {
