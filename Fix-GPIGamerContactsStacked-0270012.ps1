...
$ExpectedProductionVersion = "0.27.0.11"
$ExpectedTestVersion = "0.8.0.11"
$NewProductionVersion = "0.27.0.12"
$NewTestVersion = "0.8.0.12"
...
function Ensure-ChildText {
    ...
    if ($null -eq $Child) {
        $Child = New-RdlElement -Doc $Doc -LocalName $LocalName
...
function Replace-GamerContactsParagraphs {
    ...
    $ContactExpression = '=Trim(IIF(Len(Trim(First(Fields!GamerContactName1.Value,"DataSet_Result"))) > 0, First(Fields!GamerContactName1.Value,"DataSet_Result"), "") & IIF(Len(Trim(First(Fields!GamerContactName2.Value,"DataSet_Result"))) > 0, vbCrLf & First(Fields!GamerContactName2.Value,"DataSet_Result"), ""))'
    ...
    Ensure-ChildText -Doc $Doc -Parent $Textbox -LocalName "CanGrow" -Value "true" -NsMgr $NsMgr
...
function Update-GamerContactsLayout {
    ...
    foreach ($TextboxNode in $Textboxes) {
        ...
        if (($Text -match 'Gamer Contacts') -and ($Text -match 'GamerContactName1')) {
...
$BackupRoot = Join-Path -Path $RepoRoot -ChildPath ".gpi-backups\gamer-contacts-stacked-0270012-$Timestamp"
...
### Fixed
- Gamer Contacts now render as a stacked block:
  Gamer Contacts:
  Contact Name 1
  Contact Name 2
- This removes the inline first contact name after the label and keeps both names left-aligned under the label.
...
try {
    ...
    Write-Host " GPI Gamer Contacts stacked layout pass 0.27.0.12" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "The Gamer Contacts stacked layout build failed. Restoring modified files." -ForegroundColor Red
...
Write-Host " GPI 0.27.0.12 build passed" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
...
Write-Host "Publish production first, then tests only after production 0.27.0.12 is installed in the sandbox." -ForegroundColor Yellow
