...
function Find-SalesOrderLayout {
    param([Parameter(Mandatory)][string]$Folder)

    $Preferred = Join-Path -Path $Folder -ChildPath "GPISalesOrderConfirmationBranded.rdl"
    if (Test-Path -LiteralPath $Preferred) {
        return $Preferred
    }

    $Matches = Get-ChildItem -LiteralPath $Folder -File -Filter "*.rdl" |
        Where-Object { $_.Name -like "*Sales*Order*Confirmation*.rdl" -or $_.Name -like "*OrderConfirmation*.rdl" } |
...
function Test-GoodSalesOrderLayout {
    ...
    $LayoutPath = Find-SalesOrderLayout -Folder $Folder
    if ([string]::IsNullOrWhiteSpace($LayoutPath)) {
        $Available = (Get-ChildItem -LiteralPath $Folder -File -Filter "*.rdl" | Select-Object -ExpandProperty Name) -join ", "
        throw "Sales Order layout was not found in $Label folder $Folder. Available RDL files: $Available"
    }
    ...
    if ($Problems.Count -gt 0) {
        throw "Sales Order layout validation failed for $Label ($LayoutPath): $($Problems -join '; ')"
    }
...
$ValidatedBackupSalesOrderLayout = Test-GoodSalesOrderLayout -Folder $BackupReportLayoutFolder -Label "backup"

$SafetyBackup = Join-Path -Path $BackupBase -ChildPath ("restore-exact-good-backup-to-0270016-v2-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
New-Item -ItemType Directory -Path $SafetyBackup -Force | Out-Null

$FilesToSafetyBackup = @($ProductionAppJson, $TestAppJson, $ChangeLog) + (Get-ChildItem -LiteralPath $ReportLayoutFolder -File -Filter "*.rdl" | ForEach-Object { $_.FullName })
...
try {
    # Restore every backup RDL by enumerating files. Do not use -LiteralPath with a wildcard.
    $BackupLayoutFiles = Get-ChildItem -LiteralPath $BackupReportLayoutFolder -File -Filter "*.rdl"
    if ($BackupLayoutFiles.Count -eq 0) {
        throw "No RDL files were found in backup layout folder: $BackupReportLayoutFolder"
    }

    foreach ($BackupLayoutFile in $BackupLayoutFiles) {
        $Destination = Join-Path -Path $ReportLayoutFolder -ChildPath $BackupLayoutFile.Name
        Copy-Item -LiteralPath $BackupLayoutFile.FullName -Destination $Destination -Force
    }
...
### Restored
- Restored RDLC layouts from exact backup:
  $GoodBackupName
...
"@
    ...
    $ValidatedRestoredSalesOrderLayout = Test-GoodSalesOrderLayout -Folder $ReportLayoutFolder -Label "restored current source"
    ...
    Write-Host " GPI exact restore to verified 0.27.0.11 layout state v2" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host "Restored from:       $GoodBackupRoot"
    Write-Host "Backup SO layout:    $ValidatedBackupSalesOrderLayout"
    Write-Host "Current SO layout:   $ValidatedRestoredSalesOrderLayout"
