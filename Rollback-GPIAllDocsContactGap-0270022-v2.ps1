...
$CurrentLayoutRoot = Join-Path -Path $ProductionRoot -ChildPath "src\reportlayout"

foreach ($RequiredPath in @($ProductionAppJson, $TestAppJson, $ChangeLog, $BuildScript, $BackupBase, $CurrentLayoutRoot)) {
...
$AllowedProductionVersions = @("0.27.0.20", "0.27.0.21", "0.27.0.22")
$AllowedTestVersions = @("0.8.0.20", "0.8.0.21", "0.8.0.22")

if ($AllowedProductionVersions -notcontains [string]$CurrentProductionApp.version) {
    throw "Expected current production source version 0.27.0.20, 0.27.0.21, or 0.27.0.22 before rollback, but found $($CurrentProductionApp.version). No files were changed."
}

if ($AllowedTestVersions -notcontains [string]$CurrentTestApp.version) {
    throw "Expected current test source version 0.8.0.20, 0.8.0.21, or 0.8.0.22 before rollback, but found $($CurrentTestApp.version). No files were changed."
}
...
$SafetyBackupRoot = Join-Path -Path $BackupBase -ChildPath "idempotent-rollback-all-docs-contact-gap-to-0270022-$Timestamp"
...
foreach ($BackupLayout in $BackupLayouts) {
    ...
    $CurrentLayoutPath = Join-Path -Path $CurrentLayoutRoot -ChildPath $RelativeFromBackupLayoutRoot
...
try {
    foreach ($BackupLayout in $BackupLayouts) {
        $RelativeFromBackupLayoutRoot = $BackupLayout.FullName.Substring($BackupLayoutRoot.Length).TrimStart('\')
        $DestinationLayout = Join-Path -Path $CurrentLayoutRoot -ChildPath $RelativeFromBackupLayoutRoot
...
- Restored affected RDLC layouts from the pre-0.27.0.21 backup:
  $($SourceBackup.Name)
...
"@
    ...
    Write-Host " GPI idempotent rollback all-doc contact gap to 0.27.0.20 state" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host "Current source was:  $($CurrentProductionApp.version) / $($CurrentTestApp.version)"
    ...
    Write-Host ""
    Write-Host "Rollback build failed. Restoring current source snapshot." -ForegroundColor Red
