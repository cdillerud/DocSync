...
async def _get_alias_metrics_safe(db, total_docs: int) -> dict:
    """Return authoritative alias metrics while keeping the dashboard resilient."""
    # Retain the existing helper signature for dashboard compatibility.
    # The authoritative service owns the response contract and calculations.
    _ = db, total_docs

    try:
        from services.vendor_alias_learning_service import get_alias_metrics
        return await get_alias_metrics()
    except Exception:
        return {
            "total_aliases": 0,
            "auto_learned": 0,
            ...
            "direct_match_rate": 0,
            "vendor_match_rate": 0,
            "vendor_resolution_rate": 0,
            "alias_matched_docs": 0,
            "direct_matched_docs": 0,
            "fuzzy_matched_docs": 0,
            "auto_resolved_docs": 0,
            "total_with_vendor": 0,
            "total_docs": 0,
        }
