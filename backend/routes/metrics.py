"""
Automation/vendor/alias/shadow-mode/extraction-quality metrics and reporting.

Extracted VERBATIM from server.py during the routes/ migration (see
MIGRATION_PROGRESS.md at repo root, Group 16). No logic changed.
"""
import re
...
from core.config import EMAIL_POLLING_ENABLED, EMAIL_POLLING_USER, EMAIL_POLLING_INTERVAL_MINUTES
from core.job_config import DEFAULT_JOB_TYPES, TransactionAction, DRAFT_CREATION_CONFIG
...
async def _get_automation_metrics_internal(days: int = 30, job_type: str = None):
    ...
    return {
        ...
        "draft_feature_enabled": config.ENABLE_CREATE_DRAFT_HEADER,
...
async def get_shadow_mode_status():
    ...
    return {
        "feature_flags": {
            "CREATE_DRAFT_HEADER": config.ENABLE_CREATE_DRAFT_HEADER,
        ...
        "shadow_mode": {
            ...
            "is_active": settings.get("shadow_mode_started_at") is not None and not config.ENABLE_CREATE_DRAFT_HEADER
        },
