...
async def get_order_intake_status():
    ...
    return {
        ...
        "message": (
            ...
            if writes_enabled()
            else "Preflight and approval are enabled; BC writes are disabled"
        ),
    }
...
async def get_order_intake_document(
    ...
    """Return the source document and its deterministic candidate."""

    try:
        preflight = (
            await run_shadow_preflight(_db(), document_id)
...
async def preflight_pending_orders(
    ...
    """Evaluate and persist pending orders without writing to BC."""

    return await batch_preflight_pending(_db(), limit=limit)


@router.post("/order-intake/{document_id}/preflight")
async def preflight_order(document_id: str):
    """Evaluate and persist a single sales-order candidate."""

    try:
        return await run_shadow_preflight(_db(), document_id)
    except SalesOrderDocumentNotFound:
        raise _not_found(document_id)
...
async def approve_order(document_id: str, request: ApprovalRequest):
    """Record human approval and rerun deterministic preflight."""
