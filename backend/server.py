...
# MOVED to services/mailbox_polling_engine.py and routes/mailbox_sources.py
# during routes/ migration (see MIGRATION_PROGRESS.md, Group 17). Imported
# back here because dynamic_mailbox_polling_worker (stays in server.py -
# background task lifecycle) still calls poll_mailbox_for_documents.
from services.mailbox_polling_engine import poll_mailbox_for_documents
from routes.mailbox_sources import router as mailbox_sources_router

# ==================== BC SANDBOX ====================
# MOVED to routes/bc_sandbox.py during routes/ migration (see
# MIGRATION_PROGRESS.md, Group 18). 14 endpoints.
from routes.bc_sandbox import router as bc_sandbox_router

# ==================== SALES FILE IMPORT ====================
# MOVED to routes/sales_file_import.py during routes/ migration (see
# MIGRATION_PROGRESS.md, Group 20). 6 endpoints.
from routes.sales_file_import import router as sales_file_import_router
...
# ==================== SETTINGS ====================
# MOVED to routes/config.py during routes/ migration (see
# MIGRATION_PROGRESS.md, Group 8 - the deliberately-last group). Imported
# back here because the startup event still calls _load_config_from_db()
# once. core.config is now the single source of truth for all these
# values everywhere in the app - no more dual-write needed.
from routes.config import router as config_router, _load_config_from_db
...
app.include_router(mailbox_sources_router)
app.include_router(bc_sandbox_router)
app.include_router(sales_file_import_router)
...
async def startup():
    ...
    logger.info("GPI Document Hub started. Demo mode: %s, Loaded %d vendor aliases", _core_config.DEMO_MODE, len(aliases))
