...
async def run_upload_and_link_workflow(doc_id: str, file_content: bytes, file_name: str, doc_type: str, bc_record_id: str = None, bc_document_no: str = None):
    """Compatibility wrapper. Authoritative: services.document_linking.run_upload_and_link_workflow"""
    from services.document_linking import run_upload_and_link_workflow as _run
    return await _run(doc_id, file_content, file_name, doc_type, bc_record_id, bc_document_no)
...
async def classify_document_type(
    ...
    """Compatibility wrapper. Authoritative: services.document_classification.classify_document_type"""
    from services.document_classification import classify_document_type as _classify
    return await _classify(document, extracted_fields, suggested_type, confidence, metadata)


def _get_category_for_doc_type(doc_type: str) -> str:
    """Compatibility wrapper. Authoritative: services.document_classification.get_category_for_doc_type"""
    from services.document_classification import get_category_for_doc_type
    return get_category_for_doc_type(doc_type)
...
# poll_mailbox_for_documents: Authoritative source is now services.mailbox_polling


async def poll_mailbox_for_documents(mailbox_address: str, default_category: str = "AP", source_id: str = None):
    """Compatibility wrapper. Authoritative: services.mailbox_polling.poll_mailbox_for_documents"""
    from services.mailbox_polling import poll_mailbox_for_documents as _poll
    return await _poll(mailbox_address, default_category, source_id)
