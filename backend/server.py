...
# ---------------------------------------------------------------------------
# COMPATIBILITY WRAPPER: workflow-audited document linking
# Authoritative source: services.workflow_link_service.link_document
# ---------------------------------------------------------------------------
async def link_document(doc_id: str):
    """Compatibility wrapper for workflow-audited document linking."""
    from services.workflow_link_service import link_document as _impl
...
async def intake_document(
    ...
    if sp_result and (decision == "auto_link" or decision == "auto_create"):
        ...
        if is_draft_eligible and suggested_type == "AP_Invoice":
            ...
            vendor_info = validation_results.get("bc_record_info") or {}
            vendor_no = vendor_info.get("number", "")
            normalized_fields = validation_results.get("normalized_fields") or {}
...
async def startup():
    ...
    async def _captured_retry_scheduler():
