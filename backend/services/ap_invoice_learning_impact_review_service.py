"""
GPI Document Hub — AP Invoice Learning Apply-Impact Review Service

Measures whether applied learning suggestions improve future reviewer
agreement for the affected vendors and suggestion types.
...
async def run_ap_learning_impact_review(
    db,
    ...
    """Compare pre-apply vs post-apply outcomes for applied AP suggestions."""
    ...
    applied = await db.ap_learning_suggestions.find(match, {"_id": 0}).to_list(500)
    if not applied:
        return {"total_applied": 0, "message": "No applied AP suggestions found"}

    affected_vendors = list({s.get("vendor_no") for s in applied if s.get("vendor_no")})
    all_feedback = await db.ap_reviewer_feedback.find(
        {"vendor_no": {"$in": affected_vendors}}, {"_id": 0}
    ).to_list(5000)

    by_type = defaultdict(lambda: {"pre": [], "post": [], "applied_count": 0})
    by_vendor = defaultdict(lambda: {"pre": [], "post": [], "suggestions": []})
    improved = []
    no_change = []
    regressed = []

    for suggestion in applied:
        vno = suggestion.get("vendor_no", "")
        stype = suggestion.get("suggestion_type", "")
        applied_at = suggestion.get("applied_at", "")

        vendor_fb = [fb for fb in all_feedback if fb.get("vendor_no") == vno]
        pre = [fb for fb in vendor_fb if (fb.get("timestamp") or "") < applied_at]
        post = [fb for fb in vendor_fb if (fb.get("timestamp") or "") >= applied_at]

        by_type[stype]["pre"].extend(pre)
        by_type[stype]["post"].extend(post)
        by_type[stype]["applied_count"] += 1

        by_vendor[vno]["pre"].extend(pre)
        by_vendor[vno]["post"].extend(post)
        by_vendor[vno]["suggestions"].append(suggestion)
    ...
    for stype, data in by_type.items():
        ...
        pre_disagree = _top_disagreed_fields(data["pre"])
        post_disagree = _top_disagreed_fields(data["post"])
        ...
        type_summary[stype] = {
            ...
            "pre_top_disagreed": pre_disagree[:3],
            "post_top_disagreed": post_disagree[:3],
        }

    vendor_summary = []
    for vno, data in sorted(by_vendor.items(), key=lambda x: len(x[1]["suggestions"]), reverse=True):
        pre_rate = _agreement_rate(data["pre"])
        post_rate = _agreement_rate(data["post"])
        vendor_summary.append({
            "vendor_no": vno,
            "vendor_name": data["suggestions"][0].get("vendor_name", "") if data["suggestions"] else "",
    ...
    return {
        ...
        "filters_applied": {k: v for k, v in {
            ...
            "vendor_no": vendor_no, "suggestion_type": suggestion_type,
            "applied_by": applied_by,
        }.items() if v},
    }


async def get_ap_impact_details(
    ...
    total = await db.ap_learning_apply_audit.count_documents(match)
