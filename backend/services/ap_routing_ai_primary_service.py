...
def _augment_prompt_with_train_context(prompt: str, learning_context: Optional[Dict[str, Any]]) -> str:
    ...
    learned_rules = (
        ...
        "Dropship International/<reference> or Warehouse International/<reference>. For a NEW current child leaf, "
        "generalize a dynamic-child pattern only when the relevant TRAIN dynamic_route_usage block contains at least "
        "two distinct entries in dynamic_children. A single historical dynamic child is not a reusable pattern and "
        "must not justify inventing a different child. If the exact current child route itself appears in dynamic_children, "
        "that exact observed child may still be considered. Otherwise, when comparable human labels use the parent, keep "
        "the parent.\n"
