...
def test_excluded_source_is_blocked_even_as_supported_purchase_order():
    from services.sales_order_runtime import prepare_sales_order_document

    doc = valid_doc()
    doc.update(
        {
            "sales_order_excluded": True,
            # Contains an AP-only term, so _remove_ap_only_messages drops the
            # upstream validation copy of this reason.
            "sales_order_exclusion_reason": "Vendor name matches a Gamer supplier.",
        }
    )

    prepared = prepare_sales_order_document(doc)
    result = preflight_sales_order(prepared)

    assert prepared["doc_type"] == "PURCHASE_ORDER"
