...
import {
  LayoutDashboard, UploadCloud, Files, Settings, Moon, Sun, LogOut, Menu, X, Brain,
  FileSpreadsheet, ChevronRight, FolderSync, ClipboardCheck
} from 'lucide-react';
...
const navItems = [
  ...
  { to: '/sales/order-review', icon: ClipboardCheck, label: 'Sales Order Review' },
...
export default function AppLayout() {
    ...
    const fetchBCStatus = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setBcStatus({
            loading: false,
            connected: isNotDemoMode && hasCredentials,
        ...
        if (settingsRes.ok) {
          ...
          const hasSecret =
            bcConfig.client_secret_set || bcConfig.has_secret;
          setBcStatus({
            loading: false,
        ...
        console.warn('[BC Status] Status lookup failed:', error.message);
        setBcStatus({
          loading: false,
          connected: false,
          demoMode: true,
  ...
  const getPageTitle = () => {
    ...
    if (path === '/sales/order-review') return 'Sales Order Review';
    ...
    if (path === '/migration/onegamer-poc') {
      return 'SharePoint Migration POC';
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden" data-testid="main-layout">
      ...
      <aside
        className={`glass-sidebar fixed md:static inset-y-0 left-0 z-50 w-64 border-r border-border flex flex-col transform transition-transform duration-200 md:translate-x-0 ${
      ...
      >
        ...
        <div className="p-3 border-t border-border shrink-0">
          <div
            ...
            data-testid="bc-status-indicator"
          >
            {bcStatus.loading ? (
              <>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header
          className="h-16 bg-background/80 backdrop-blur-md border-b border-border/50 sticky top-0 z-30 flex items-center justify-between px-4 md:px-6 shrink-0"
          data-testid="header"
        >
