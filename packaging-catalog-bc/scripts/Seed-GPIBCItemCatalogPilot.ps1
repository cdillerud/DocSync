...
$planPath = Join-Path $DiscoveryFolder 'BC_Packaging_Catalog_Pilot_Seed_Plan.csv'
$manifestPath = Join-Path $DiscoveryFolder 'BC_Packaging_Catalog_Pilot_Seed_Manifest.csv'
...
foreach ($category in $quotas.Keys) {
    ...
    $categoryRows = @(
        $safeSource |
            Where-Object { ([string]$_.ItemCategoryCode).ToUpperInvariant() -eq $category } |
            Sort-Object `
    ...
    foreach ($row in @($categoryRows | Select-Object -First $quota)) {
...
Write-Host ''
Write-Host 'GPI PACKAGING CATALOG PILOT SEED' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'PILOT PLAN' -ForegroundColor Cyan
$plan |
    Select-Object ItemCategoryCode, GamerID, VendorNo, CurrentSupplierUnitCost, Material, Capacity, CapacityUOM, Finish, Color, Style, GramWeight |
...
if (-not $Execute -and -not $Cleanup) {
    ...
    Write-Host 'After reviewing the plan, rerun with -Execute to create only these pilot records in the sandbox.'
    return
}
...
try {
    ...
    if ($Cleanup) {
        ...
        Write-Host 'PILOT CLEANUP COMPLETE' -ForegroundColor Green
        return
    }
    ...
    foreach ($row in $selected) {
        ...
        $payload = [ordered]@{
            ...
            priceChangeNote = 'Milestone 10 pilot seed from BC Item Last Direct Cost; source effective date unknown.'
            blocked = $false
        }
