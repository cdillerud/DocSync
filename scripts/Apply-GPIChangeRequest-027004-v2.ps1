...
function InsertEnabledIntoAction {
    ...
    if (($Matches.Count -lt $MinimumMatches) -or ($Matches.Count -gt $MaximumMatches)) {
        throw "Expected $MinimumMatches-$MaximumMatches action match(es) for $Description, but found $($Matches.Count). No files were changed."
    }
...
function InsertActionAfterAction {
    ...
    if ($Content -match [regex]::Escape(($NewActionText -split "`r?`n", 2, "RegexMatch")[0])) {
        return $Content
    }
...
$BackupRoot = Join-Path $RepoRoot ".gpi-backups\change-request-027004-v2-$Timestamp"
...
try {
    ...
    Write-Host " GPI final change request 0.27.0.4 v2" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "The 0.27.0.4 v2 change-request build failed. Restoring all modified files." -ForegroundColor Red
