...
function InsertActionAfterAction {
    ...
    if (($FirstMeaningfulLine <> '') -and ($Content -match [regex]::Escape($FirstMeaningfulLine))) {
        return $Content
    }
...
$BackupRoot = Join-Path $RepoRoot ".gpi-backups\change-request-027004-v3-$Timestamp"
...
try {
    ...
    Write-Host " GPI final change request 0.27.0.4 v3" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "The 0.27.0.4 v3 change-request build failed. Restoring all modified files." -ForegroundColor Red
