...
$SalesOrderLineEnding = Get-LineEnding -Content $UpdatedSalesOrder

$EmailPickTicketPattern = '(\r?\n\s*action\(GPIEmailPickTicket\)\s*\{\r?\n)'
$EmailPickTicketCount = ([regex]::Matches($UpdatedSalesOrder, $EmailPickTicketPattern)).Count
if ($EmailPickTicketCount -ne 1) {
    throw "Expected exactly one Gamer Email Pick Ticket action, but found $EmailPickTicketCount. No files were changed."
}
$UpdatedSalesOrder = $UpdatedSalesOrder -replace $EmailPickTicketPattern, ('$1                    Enabled = IsPickTicketAvailable;' + $SalesOrderLineEnding)

$PreviewPickTicketPattern = '(\r?\n\s*action\(GPI(?:PreviewOwnedPickTicket|PreviewPickTicket)\)\s*\{\r?\n)'
$PreviewPickTicketCount = ([regex]::Matches($UpdatedSalesOrder, $PreviewPickTicketPattern)).Count
if ($PreviewPickTicketCount -ne 1) {
    throw "Expected exactly one Gamer Preview Pick Ticket action, but found $PreviewPickTicketCount. No files were changed."
}
$UpdatedSalesOrder = $UpdatedSalesOrder -replace $PreviewPickTicketPattern, ('$1                    Enabled = IsPickTicketAvailable;' + $SalesOrderLineEnding)
...
$FinalBraceIndex = $UpdatedSalesOrder.LastIndexOf('}')
if ($FinalBraceIndex -lt 0) {
    throw "Could not find the final Sales Order page-extension closing brace. No files were changed."
}
$UpdatedSalesOrder =
    $UpdatedSalesOrder.Substring(0, $FinalBraceIndex) +
    $SalesOrderStateInsertion +
    $SalesOrderLineEnding +
    $UpdatedSalesOrder.Substring($FinalBraceIndex)

$UpdatedPostedSalesInvoices = $Originals[$PostedSalesInvoicesPageExt]
$PostedInvoiceLineEnding = Get-LineEnding -Content $UpdatedPostedSalesInvoices
...
$InvoiceQueueActionPattern = "(?s)(\r?\n\s*action\(GPIOpenInvoiceQueue\)\s*\{.*?ToolTip = 'Opens the Gamer filterable end-of-day posted sales invoice email queue\.';\s*\})"
$InvoiceQueueMatches = [regex]::Matches($UpdatedPostedSalesInvoices, $InvoiceQueueActionPattern)
if ($InvoiceQueueMatches.Count -ne 1) {
    throw "Expected exactly one Posted Sales Invoices queue action, but found $($InvoiceQueueMatches.Count). No files were changed."
}

if ($UpdatedPostedSalesInvoices -match 'action\(GPIOpenEndOfDayInvoiceBatch\)') {
    throw "The Gamer EOD Invoice Batch action already exists. No files were changed."
}

$UpdatedPostedSalesInvoices = [regex]::Replace(
    $UpdatedPostedSalesInvoices,
    $InvoiceQueueActionPattern,
    '${1}' + $EodAction)
...
$BackupRoot = Join-Path $RepoRoot ".gpi-backups\change-request-027004-v7-$Timestamp"
...
try {
    ...
    Write-Host " GPI final change request 0.27.0.4 v7" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "The 0.27.0.4 v7 change-request build failed. Restoring all modified files." -ForegroundColor Red
