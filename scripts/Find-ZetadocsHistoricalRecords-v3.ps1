...
.DESCRIPTION
  Version 3. Read-only. Uses Microsoft Graph delegated permissions Sites.Read.All and
...
if ($LibraryName) {
    $selectedDrives = @(
        $drives | Where-Object {
            $driveName = [string](Get-ObjectPropertyValue -Object $_ -Name 'name')
            $driveName -eq $LibraryName -or $driveName -like $LibraryName
        }
    )
    ...
    if (-not $selectedDrives.Count) {
        ...
        throw "Library '$LibraryName' not found. Available libraries: $($availableLibraries -join ', ')"
    }
}
else {
    ...
    $selectedDrives = @(
        $drives | Where-Object {
            $driveName = [string](Get-ObjectPropertyValue -Object $_ -Name 'name')
            $driveName -like "$RootFolder*"
        }
    )
}

foreach ($selectedDrive in $selectedDrives) {
    $selectedDriveId = [string](Get-ObjectPropertyValue -Object $selectedDrive -Name 'id')
    $selectedDriveName = [string](Get-ObjectPropertyValue -Object $selectedDrive -Name 'name')
    $selectedDriveWebUrl = [string](Get-ObjectPropertyValue -Object $selectedDrive -Name 'webUrl')

    if ([string]::IsNullOrWhiteSpace($selectedDriveId)) {
        Write-Warning 'Skipping a SharePoint library because Microsoft Graph did not return its drive ID.'
        continue
    }

    if ([string]::IsNullOrWhiteSpace($selectedDriveName)) {
        $selectedDriveName = $selectedDriveId
    }
    ...
    $driveRootId = [string](Get-ObjectPropertyValue -Object $driveRoot -Name 'id')
    if ([string]::IsNullOrWhiteSpace($driveRootId)) {
        Write-Warning "Skipping SharePoint library '$selectedDriveName' because its root item ID was not returned."
        continue
    }
    ...
    $scanTargets.Add(
        [pscustomobject]@{
            ...
            LibraryName = $selectedDriveName
            LibraryWebUrl = $selectedDriveWebUrl
            RootId = $driveRootId
...
if (-not $scanTargets.Count) {
    ...
    foreach ($candidateDrive in $drives) {
        try {
            $candidateDriveId = [string](Get-ObjectPropertyValue -Object $candidateDrive -Name 'id')
            $candidateDriveName = [string](Get-ObjectPropertyValue -Object $candidateDrive -Name 'name')
            $candidateDriveWebUrl = [string](Get-ObjectPropertyValue -Object $candidateDrive -Name 'webUrl')

            if ([string]::IsNullOrWhiteSpace($candidateDriveId)) {
                continue
            }

            if ([string]::IsNullOrWhiteSpace($candidateDriveName)) {
                $candidateDriveName = $candidateDriveId
            }

            $folderRoot = Invoke-MgGraphRequest `
                -Method GET `
                -Uri ("https://graph.microsoft.com/v1.0/drives/{0}/root:/{1}?`$select=id,name,webUrl,folder" -f $candidateDriveId,$encodedRoot) `
                -OutputType PSObject `
                -ErrorAction Stop

            $rootFolderFacet = Get-ObjectPropertyValue -Object $folderRoot -Name 'folder'
            $folderRootId = [string](Get-ObjectPropertyValue -Object $folderRoot -Name 'id')
            if (($null -ne $rootFolderFacet) -and -not [string]::IsNullOrWhiteSpace($folderRootId)) {
                $scanTargets.Add(
                    [pscustomobject]@{
                        DriveId = $candidateDriveId
                        LibraryName = $candidateDriveName
                        LibraryWebUrl = $candidateDriveWebUrl
                        RootId = $folderRootId
...
if (-not $scanTargets.Count) {
    ...
    throw "No Zetadocs libraries or '$RootFolder' folders were found. Libraries checked: $($availableLibraries -join ', ')"
}

Write-Host '[FOUND] Scan targets:'
foreach ($target in $scanTargets) {
    Write-Host ("  - Library: {0}; Start: {1}" -f $target.LibraryName,$target.DisplayRoot)
}
...
foreach ($target in $scanTargets) {
    ...
    $driveId = [string]$target.DriveId
    $libraryName = [string]$target.LibraryName
    $libraryWebUrl = [string]$target.LibraryWebUrl
    $rootItemId = [string]$target.RootId

    if ([string]::IsNullOrWhiteSpace($driveId) -or
        [string]::IsNullOrWhiteSpace($rootItemId)) {

        Write-Warning "Skipping scan target '$libraryName' because its Graph identifiers are incomplete."
        continue
    }

    $queue = [System.Collections.Generic.Queue[object]]::new()
    $queue.Enqueue([pscustomobject]@{ Id=$rootItemId; RelativePath='' })

    while ($queue.Count -gt 0 -and -not $stop) {
        $folder = $queue.Dequeue()
        $folderCount++
        $childrenUri = "https://graph.microsoft.com/v1.0/drives/$driveId/items/$($folder.Id)/children?`$select=id,name,size,webUrl,createdDateTime,lastModifiedDateTime,file,folder"
        $children = @(Get-GraphPages -Uri $childrenUri)

        foreach ($child in $children) {
            $childName = [string](Get-ObjectPropertyValue -Object $child -Name 'name')
            $childId = [string](Get-ObjectPropertyValue -Object $child -Name 'id')

            if ([string]::IsNullOrWhiteSpace($childName) -or
                [string]::IsNullOrWhiteSpace($childId)) {

                Write-Warning "Skipping an item in library '$libraryName' because Graph returned it without a name or ID."
                continue
            }

            $relative = if ($folder.RelativePath) {
                "$($folder.RelativePath)/$childName"
            }
            else {
                $childName
            }
            ...
            $candidate = Get-RecordCandidate -FileName $childName -Area $details.Area
            ...
            $childSize = Get-ObjectPropertyValue -Object $child -Name 'size'
            $childCreatedDateTime = Get-ObjectPropertyValue -Object $child -Name 'createdDateTime'
            $childLastModifiedDateTime = Get-ObjectPropertyValue -Object $child -Name 'lastModifiedDateTime'
            $childWebUrl = Get-ObjectPropertyValue -Object $child -Name 'webUrl'

            $sizeBytes = 0
            if ($null -ne $childSize) {
                $sizeBytes = [Int64]$childSize
            }

            $files.Add([pscustomobject]@{
                LibraryName = $libraryName
                LibraryWebUrl = $libraryWebUrl
                ...
                RelativePath = $relative
                SizeBytes = $sizeBytes
                CreatedDateTime = [string]$childCreatedDateTime
                LastModifiedDateTime = [string]$childLastModifiedDateTime
                SharePointUrl = [string]$childWebUrl
            })
...
Site:            $SiteUrl
Libraries:       $((@($scanTargets | ForEach-Object { $_.LibraryName }) | Sort-Object -Unique) -join ", ")
