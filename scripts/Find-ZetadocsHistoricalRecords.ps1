...
$candidates = $drives
if ($LibraryName) {
    $candidates = @($drives | Where-Object { $_.name -eq $LibraryName -or $_.name -like $LibraryName })
...
foreach ($drive in $candidates) {
    try {
        $root = Invoke-GraphGet -Attempts 2 -Uri ("https://graph.microsoft.com/v1.0/drives/{0}/root:/{1}?`$select=id,name,webUrl,folder" -f $drive.id,$encodedRoot)
        $rootFolderFacet = Get-ObjectPropertyValue -Object $root -Name 'folder'
        if ($null -ne $rootFolderFacet) { $matches.Add([pscustomobject]@{ Drive=$drive; Root=$root }) }
    }
    catch { }
}

if (-not $matches.Count) {
    throw "Folder '$RootFolder' was not found. Libraries checked: $(($drives.name | Sort-Object) -join ', ')"
}
if ($matches.Count -gt 1 -and -not $LibraryName) {
    throw "More than one library contains '$RootFolder': $(($matches.Drive.name | Sort-Object) -join ', '). Run again with -LibraryName."
}

$drive = $matches[0].Drive
$rootItem = $matches[0].Root
Write-Host "[FOUND] Library: $($drive.name)"
Write-Host "[FOUND] Folder:  $RootFolder"
...
while ($queue.Count -gt 0 -and -not $stop) {
    ...
    foreach ($child in $children) {
        ...
        if ($MaxFiles -gt 0 -and $files.Count -ge $MaxFiles) { $stop = $true; break }
    }
}
