...
$ProcedureOpenBrace = $TestContent.IndexOf('{', $ProcedureMatch.Index)
if ($ProcedureOpenBrace -lt 0) {
    # AL procedure bodies use begin/end rather than braces. Find the next test.
    $NextTestMatch = [regex]::Match(
        $TestContent,
        '(?im)^\s*\[Test\]',
        $ProcedureMatch.Index + $ProcedureMatch.Length
    )
    ...
    $ProcedureEnd = Get-MatchingBraceIndex -Text $TestContent -OpenBraceIndex $ProcedureOpenBrace
}
...
$UpdatedProcedure = [regex]::Replace(
    $ProcedureBlock,
    $CustomerCardDeclarationPattern,
