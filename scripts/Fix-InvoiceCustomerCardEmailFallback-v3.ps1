...
function Replace-ProcedureByName {
    param(
    ...
    if ($StartIndexes.Count -ne 1) {
        throw "Expected exactly one local procedure named $ProcedureName, but found $($StartIndexes.Count). No files were changed."
    }
...
# The two earlier patch attempts stopped before writing anything. These hashes
# verify the two business-logic files still match the captured local source.
Assert-FileHash `
...
$UsedTestObjectIds = @()
Get-ChildItem -LiteralPath $TestRoot -Recurse -File -Filter "*.al" |
    ForEach-Object {
        $Text = Get-Content -LiteralPath $_.FullName -Raw
        foreach ($Match in [regex]::Matches($Text, '(?im)^\s*codeunit\s+(\d+)\b')) {
            $UsedTestObjectIds += [int]$Match.Groups[1].Value
        }
    }

if ($UsedTestObjectIds -contains 70719) {
    throw "Test codeunit ID 70719 is already in use. No files were changed."
}
...
$UpdatedDocumentPolicy = Replace-ProcedureByName `
    -Source $OriginalDocumentPolicy `
    -ProcedureName "GetCustomerPrimaryContactEmail" `
...
$BackupRoot = Join-Path $RepoRoot ".gpi-backups\fix-invoice-customer-email-fallback-v3-$Timestamp"
...
try {
    ...
    Write-Host " Invoice Customer Card email fallback v3" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
