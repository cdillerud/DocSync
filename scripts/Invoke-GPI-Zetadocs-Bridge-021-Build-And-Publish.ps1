...
$BuildUrl = "$RawBase/Build-GPI-Zetadocs-First-Batch-Stale-Recovery-Bridge-0.1.0.21-REV4.ps1"
...
$BuildScript = Join-Path $TempRoot 'Build-GPI-Zetadocs-First-Batch-Stale-Recovery-Bridge-0.1.0.21-REV4.ps1'
...
if ($Branch -ne $ExpectedBranch) { throw "SAFETY STOP: expected branch '$ExpectedBranch', found '$Branch'." }
if (-not (Test-Path -LiteralPath $BridgeRoot)) { throw "Bridge source not found: $BridgeRoot" }
...
foreach ($Path in @($BuildScript,$PublishScript)) {
    ...
    if($Errors.Count -gt 0){throw "Downloaded script failed parser validation: $Path :: $(($Errors.Message)-join '; ')"}
}
Write-Host 'Build tool   : parser PASS' -ForegroundColor Green
Write-Host 'Publish tool : parser PASS' -ForegroundColor Green

Section '3. BUILD BRIDGE .21 FROM CURRENT LOCAL SOURCE'
& $BuildScript
if($LASTEXITCODE -ne 0){throw "Bridge .21 builder exited with code $LASTEXITCODE."}
if(-not(Test-Path $PackagePath)){throw "Builder completed but package was not found: $PackagePath"}
...
if($LASTEXITCODE -ne 0){throw "Bridge .21 publisher exited with code $LASTEXITCODE."}
