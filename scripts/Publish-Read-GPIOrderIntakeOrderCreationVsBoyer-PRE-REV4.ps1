...
Write-Host 'GPI ORDER INTAKE - ORDER CREATION VS BOYER REV4 / API-ENTITY-SET FIXED' -ForegroundColor Cyan
Write-Host ('=' * 120) -ForegroundColor Cyan
...
$tempScript = Join-Path $PSScriptRoot ('.GPIOrderIntake-OrderCreationVsBoyer-REV4-' + [guid]::NewGuid().ToString('N') + '.tmp.ps1')
try {
    Set-Content -LiteralPath $tempScript -Value $patched -Encoding UTF8 -NoNewline
    Write-Host 'REV4 endpoint/comparison patches: PASS' -ForegroundColor Green
...
Write-Host ''
Write-Host 'GPI ORDER INTAKE ORDER CREATION VS BOYER REV4: WRAPPER COMPLETE' -ForegroundColor Green
