...
function Invoke-SourceScript([string]$KnownHosts,[string]$ScriptText) {
    $stderrFile = Join-Path $env:TEMP ("v108-rev4-ssh-{0}.err" -f [guid]::NewGuid().ToString('N'))
    try {
        $args = @(
            '-i',$KeyPath,
            '-o','BatchMode=yes',
...
if ($Verified) {
    ...
    Write-Host "V108_REV4_LOCAL_CORPUS=$Verified" -ForegroundColor Cyan
    Write-Host 'V108_REV4_CORPUS_SHA_MATCH=PASS' -ForegroundColor Green
}
else {
    Write-Host 'V108_REV4_LOCAL_CORPUS=NOT_FOUND; retrieving exact legacy SharePoint item read-only via source backend...' -ForegroundColor Yellow
...
async def main():
    ...
    with open('/tmp/v108-w117105.pdf','wb') as f:
        f.write(data)
    print(f'V108_REV4_GRAPH_READ_BYTES={len(data)}')
    print(f'V108_REV4_GRAPH_READ_SHA256={sha}')
    print('V108_REV4_LEGACY_SHAREPOINT_READ=PASS')
asyncio.run(main())
PY
docker cp /tmp/v108-sp-read.py "`$backend:/tmp/v108-sp-read.py" >/dev/null
rm -f /tmp/v108-sp-read.py
set +x
docker exec -w /app -e V108_DRIVE_ID="`$DRIVE_ID" -e V108_ITEM_ID="`$ITEM_ID" -e V108_EXPECTED_SHA="`$EXPECTED_SHA" "`$backend" python /tmp/v108-sp-read.py
...
echo V108_REV4_SOURCE_CORPUS_STAGED=PASS
"@
    ...
    if ($r.StdOut -notmatch 'V108_REV4_LEGACY_SHAREPOINT_READ=PASS') { throw 'Legacy SharePoint read marker missing.' }
    if ($r.StdOut -notmatch 'V108_REV4_SOURCE_CORPUS_STAGED=PASS') { throw 'Source corpus staging marker missing.' }
    ...
    $cleanup = Invoke-SourceScript -KnownHosts $KnownHosts -ScriptText "rm -f /tmp/v108-w117105-sharepoint.pdf`necho V108_REV4_SOURCE_TEMP_CLEANUP=PASS"
    ...
    Write-Host 'V108_REV4_CORPUS_SELF_RETRIEVAL=PASS' -ForegroundColor Green
    Write-Host 'V108_REV4_CORPUS_SHA_MATCH=PASS' -ForegroundColor Green
}

# The main V108 probe also executes a temporary Python file from /tmp. Patch only that invocation in a generated local copy so /app/services is importable.
...
$Replacement = 'probe_out=$(docker exec -w /app "$backend" python /tmp/v108-probe.py 2>&1)'
...
$Generated = Join-Path $ToolRoot 'Invoke-GPIHub-V108-REV4.generated.ps1'
...
Write-Host 'V108_REV4_GENERATED_PROBE_WORKDIR_PATCH=PASS' -ForegroundColor Green
Write-Host 'V108_REV4_ENTRY_WRAPPER=PASS' -ForegroundColor Cyan
& $Generated
