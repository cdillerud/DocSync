...
function Find-VerifiedLocalCorpus {
    ...
    foreach ($path in @($candidates | Select-Object -Unique)) {
        try { if ((Get-Sha $path) -eq $ExpectedSha) { return $path } } catch {}
    }
    return $null
}
...
function Invoke-SourceScript([string]$KnownHosts,[string]$ScriptText) {
    $stderrFile = Join-Path $env:TEMP ("v108-rev5-ssh-{0}.err" -f [guid]::NewGuid().ToString('N'))
    try {
...
if ($Verified) {
    ...
    Write-Host "V108_REV5_LOCAL_CORPUS=$Verified" -ForegroundColor Cyan
    Write-Host 'V108_REV5_CORPUS_SHA_MATCH=PASS' -ForegroundColor Green
}
else {
    Write-Host 'V108_REV5_LOCAL_CORPUS=NOT_FOUND; retrieving exact legacy SharePoint item read-only via source backend...' -ForegroundColor Yellow
...
# Explicit PYTHONPATH is required because Python executes helper scripts from /tmp.
docker exec -e PYTHONPATH=/app -w /app "`$backend" python -c 'import services, services.sharepoint_service; print("V108_REV5_SOURCE_IMPORT_PREFLIGHT=PASS")'
...
async def main():
    ...
    with open('/tmp/v108-w117105.pdf','wb') as f:
        f.write(data)
    print(f'V108_REV5_GRAPH_READ_BYTES={len(data)}')
    print(f'V108_REV5_GRAPH_READ_SHA256={sha}')
    print('V108_REV5_LEGACY_SHAREPOINT_READ=PASS')
asyncio.run(main())
PY
docker cp /tmp/v108-sp-read.py "`$backend:/tmp/v108-sp-read.py" >/dev/null
rm -f /tmp/v108-sp-read.py
set +x
docker exec -e PYTHONPATH=/app -w /app -e V108_DRIVE_ID="`$DRIVE_ID" -e V108_ITEM_ID="`$ITEM_ID" -e V108_EXPECTED_SHA="`$EXPECTED_SHA" "`$backend" python /tmp/v108-sp-read.py
...
echo V108_REV5_SOURCE_CORPUS_STAGED=PASS
"@
    ...
    foreach ($m in @('V108_REV5_SOURCE_IMPORT_PREFLIGHT=PASS','V108_REV5_LEGACY_SHAREPOINT_READ=PASS','V108_REV5_SOURCE_CORPUS_STAGED=PASS')) {
        if ($r.StdOut -notmatch [regex]::Escape($m)) { throw "Missing retrieval marker: $m" }
    }

    $scpArgs = @('-i',$KeyPath,'-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','-o',"UserKnownHostsFile=$KnownHosts",'-o','GlobalKnownHostsFile=NUL','-o','ConnectTimeout=20',"azureuser@${SourceIp}:/tmp/v108-w117105-sharepoint.pdf",$Exact)
    & scp.exe @scpArgs
    if ($LASTEXITCODE -ne 0) { throw "SCP of verified legacy corpus failed with exit code $LASTEXITCODE." }
    $cleanup = Invoke-SourceScript -KnownHosts $KnownHosts -ScriptText "rm -f /tmp/v108-w117105-sharepoint.pdf`necho V108_REV5_SOURCE_TEMP_CLEANUP=PASS"
    ...
    Write-Host 'V108_REV5_CORPUS_SELF_RETRIEVAL=PASS' -ForegroundColor Green
    Write-Host 'V108_REV5_CORPUS_SHA_MATCH=PASS' -ForegroundColor Green
}
...
$Needle = 'probe_out=$(docker exec "$backend" python /tmp/v108-probe.py 2>&1)'
$Replacement = 'probe_out=$(docker exec -e PYTHONPATH=/app -w /app "$backend" python /tmp/v108-probe.py 2>&1)'
if (-not $Raw.Contains($Needle)) { throw 'V108 main probe invocation changed; refusing broad patch.' }
$Patched = $Raw.Replace($Needle,$Replacement)
$Generated = Join-Path $ToolRoot 'Invoke-GPIHub-V108-REV5.generated.ps1'
...
if (@($errors).Count -gt 0) { throw ('Generated V108 parser failed: ' + ((@($errors) | ForEach-Object Message) -join '; ')) }
Write-Host 'V108_REV5_GENERATED_PROBE_PYTHONPATH_PATCH=PASS' -ForegroundColor Green
Write-Host 'V108_REV5_ENTRY_WRAPPER=PASS' -ForegroundColor Cyan
& $Generated
