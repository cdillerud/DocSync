...
function Invoke-NativeText {
    ...
    $token=[guid]::NewGuid().ToString('N');$err=Join-Path $env:TEMP "gpi-v110-$token.err.txt"
    ...
    try{$ErrorActionPreference='Continue';if($null-ne$nv){$PSNativeCommandUseErrorActionPreference=$false};$o=& $FilePath @Arguments 2>$err;$c=$LASTEXITCODE;$so=(@($o)|ForEach-Object{[string]$_})-join"`n";$se=if(Test-Path $err){Get-Content $err -Raw -ErrorAction SilentlyContinue}else{''};$r=[pscustomobject]@{ExitCode=[int]$c;StdOut=$so;StdErr=$se};if(-not$AllowFailure-and$c-ne0){throw "$FilePath failed ($c).`n$so`n$se"};return $r}
...
function Invoke-SshScript([string]$Known,[string]$Text){$token=[guid]::NewGuid().ToString('N');$err=Join-Path $env:TEMP "gpi-v110-ssh-$token.err.txt";$args=@('-i',$KeyPath,'-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','-o',"UserKnownHostsFile=$Known",'-o','GlobalKnownHostsFile=NUL','-o','ConnectTimeout=20',"azureuser@$SourceIp",'bash -s');$old=$ErrorActionPreference;$nv=Get-Variable PSNativeCommandUseErrorActionPreference -ErrorAction SilentlyContinue;$on=if($null-ne$nv){$nv.Value}else{$null};try{$ErrorActionPreference='Continue';if($null-ne$nv){$PSNativeCommandUseErrorActionPreference=$false};$o=(($Text-replace"`r`n","`n")|& ssh.exe @args 2>$err);$c=$LASTEXITCODE;$so=(@($o)|ForEach-Object{[string]$_})-join"`n";$se=if(Test-Path $err){Get-Content $err -Raw -ErrorAction SilentlyContinue}else{''};return [pscustomobject]@{ExitCode=[int]$c;StdOut=$so;StdErr=$se}}finally{$ErrorActionPreference=$old;if($null-ne$nv){$PSNativeCommandUseErrorActionPreference=$on};Remove-Item $err -Force -ErrorAction SilentlyContinue}}
...
try{
    ...
    foreach($cmd in 'ssh.exe','ssh-keygen.exe'){Require($null-ne(Get-Command $cmd -ErrorAction SilentlyContinue))"$cmd unavailable"}
...
[ -n "$backend" ] || exit 71
[ "$(docker inspect "$backend" -f '{{.Image}}')" = "$EXPECTED_IMAGE" ] || exit 72
[ "$(docker exec "$backend" sha256sum /app/services/document_intel_helpers.py|awk '{print $1}')" = "$EXPECTED_HELPER" ] || exit 73
health=$(docker exec "$backend" python -c 'import urllib.request; r=urllib.request.urlopen("http://127.0.0.1:8001/api/health",timeout=4); print(r.status)');case "$health" in 2??|3??);;*)exit 74;;esac

docker exec -e PYTHONPATH=/app -w /app -e V110_DRIVE="$DRIVE" -e V110_ITEM1="$ITEM1" -e V110_ITEM2="$ITEM2" "$backend" python - <<'PY'
PY
# All actual Python is file-based/direct so docker stdin behavior cannot affect the probe.
cat > /tmp/v110.py <<'PY'
...
DRIVE=os.environ['V110_DRIVE']; ITEMS=[os.environ['V110_ITEM1'],os.environ['V110_ITEM2']]
    ...
    async with httpx.AsyncClient(timeout=90.0,follow_redirects=True) as c:
        ...
        for item,name in zip(ITEMS,NAMES):
            ...
            try:
                ...
                inv=_INVOICE_TEXT_PATTERNS.findall(text);credit=_CREDIT_MEMO_PATTERNS.findall(text)
                candidate=None
                if not credit and len(inv)>=2:
                    candidate={'suggested_job_type':'AP_Invoice','confidence':0.92,'model':'candidate-pypdf-ap-invoice-text','indicator_count':len(inv)}
                row={'name':name,'page1_chars':len(text),'invoice_indicators':inv,'invoice_indicator_count':len(inv),'credit_indicator_count':len(credit),'baseline_guard':baseline,'candidate_guard':candidate}
                rows.append(row);print('V110_TUMALO_ROW='+json.dumps(row,sort_keys=True,default=str))
            finally:
                try:os.remove(path)
        ...
        print('V110_TUMALO_PYPDF_SEMANTICS=PASS' if broken and candidate else 'V110_TUMALO_PYPDF_SEMANTICS=INCOMPLETE')
asyncio.run(main())
PY
docker cp /tmp/v110.py "$backend:/tmp/v110.py" >/dev/null
rm -f /tmp/v110.py
set +e
out=$(docker exec -e PYTHONPATH=/app -w /app -e V110_DRIVE="$DRIVE" -e V110_ITEM1="$ITEM1" -e V110_ITEM2="$ITEM2" "$backend" python /tmp/v110.py 2>&1);code=$?
set -e
printf '%s\n' "$out"
docker exec "$backend" rm -f /tmp/v110.py >/dev/null 2>&1 || true
[ "$code" -eq 0 ] || exit 75
printf '%s\n' "$out"|grep -Fq 'V110_TUMALO_PYPDF_SEMANTICS=PASS' || exit 76
...
'@
    $r=Invoke-SshScript $Known $Remote;if($r.StdOut){Write-Host $r.StdOut};if($r.StdErr){Write-Host $r.StdErr -ForegroundColor DarkYellow};Set-Content (Join-Path $DiagDir 'v110-source.txt') $r.StdOut -Encoding utf8
    Require($r.ExitCode-eq0)"V110 failed with exit code $($r.ExitCode)";Require($r.StdOut-match'V110_TUMALO_PYPDF_SEMANTICS=PASS')'V110 candidate did not pass';Require($r.StdOut-match'V110_SOURCE_RUNTIME_UNCHANGED=PASS')'V110 runtime marker missing'
    Section 'V110 RESULT';Write-Host 'V110_TUMALO_PYPDF_AP_GUARD_CANDIDATE_TEST=PASS' -ForegroundColor Green;Write-Host "Diagnostics: $DiagDir"
}
