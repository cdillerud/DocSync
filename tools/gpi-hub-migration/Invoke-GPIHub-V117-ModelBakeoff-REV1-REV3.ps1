...
$CandidateNew = @'
        ...
        'backend/services/document_bundle_reference_service.py',
'@
Require ($Raw.Contains($CandidateOld)) 'Bakeoff REV3 authority-guard materialization anchor missing.'
...
'@
Require ($Raw.Contains($PytestOld)) 'Bakeoff REV3 pytest working-directory anchor missing.'
...
docker exec "`$backend" sh -c "cd '`$CONTAINER_STAGE' && PYTHONPATH='`$CONTAINER_STAGE:/app' python -c 'import services.ap_routing_ai_primary_service as a, services.ap_routing_business_context_service as b, services.ap_routing_model_bakeoff_service as m, services.ap_routing_authority_guard_service as g; paths=[a.__file__,b.__file__,m.__file__,g.__file__]; expected=\"/tmp/gpi-v117-model-bakeoff/services/\"; assert all(str(p).startswith(expected) for p in paths), paths; print(\"V117_BAKEOFF_IMPORT_ORIGINS=\"+\"|\".join(paths))'"
echo V117_BAKEOFF_IMPORT_ORIGIN=PASS

'@
Require ($Raw.Contains($CompileMarker)) 'Bakeoff REV3 import-origin insertion anchor missing.'
$Raw = $Raw.Replace($CompileMarker,$OriginProof)

$HeadingOld = "V117 MODEL BAKEOFF REV1 — TRAIN ONLY / READ ONLY"
$HeadingNew = "V117 MODEL BAKEOFF REV1 REV3 — TRAIN ONLY / READ ONLY"
Require ($Raw.Contains($HeadingOld)) 'Bakeoff REV3 heading anchor missing.'
$Raw = $Raw.Replace($HeadingOld,$HeadingNew)

$GeneratedController = Join-Path $ToolRoot 'Invoke-GPIHub-V117-ModelBakeoff-REV1-REV3-Generated.ps1'
...
if (@($errors).Count -gt 0) {
    ...
    throw "Bakeoff REV3 generated controller parse failed: $text"
}
...
Write-Host 'V117_BAKEOFF_REV3_AUTHORITY_GUARD_STAGED=PASS' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV3_PYTEST_CWD=CANDIDATE_ROOT' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV3_IMPORT_ORIGIN_ASSERTION=CONFIGURED' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV3_PRODUCTION_MUTATION=NONE' -ForegroundColor Green
Write-Host "V117_BAKEOFF_REV3_GENERATED_CONTROLLER=$GeneratedController"
