...
Require (Test-Path -LiteralPath $StatePath -PathType Leaf) "Bakeoff REV5 state missing: $StatePath"
...
$CandidateNew = @'
        ...
        'backend/services/document_bundle_reference_service.py',
'@
Require ($Raw.Contains($CandidateOld)) 'Bakeoff REV5 authority-guard materialization anchor missing.'
...
$BusinessNew = @'
        ...
        'backend/services/ap_routing_model_bakeoff_service.py',
'@
Require ($Raw.Contains($BusinessOld)) 'Bakeoff REV5 business-context expansion materialization anchor missing.'
...
'@
Require ($Raw.Contains($PytestOld)) 'Bakeoff REV5 pytest working-directory anchor missing.'
...
docker exec "`$backend" sh -c "PYTHONPATH='`$CONTAINER_STAGE:/app' python -m py_compile '`$CONTAINER_STAGE/services/ap_routing_authority_guard_service.py' '`$CONTAINER_STAGE/services/ap_routing_business_context_expansion_service.py'"
echo V117_BAKEOFF_REV5_ADDITIONAL_PYCOMPILE=PASS
...
'@
Require ($Raw.Contains($CompileMarker)) 'Bakeoff REV5 post-compile insertion anchor missing.'
$Raw = $Raw.Replace($CompileMarker,$PostCompile)

$HeadingOld = "V117 MODEL BAKEOFF REV1 — TRAIN ONLY / READ ONLY"
$HeadingNew = "V117 MODEL BAKEOFF REV1 REV5 — TRAIN ONLY / READ ONLY"
Require ($Raw.Contains($HeadingOld)) 'Bakeoff REV5 heading anchor missing.'
$Raw = $Raw.Replace($HeadingOld,$HeadingNew)

$GeneratedController = Join-Path $ToolRoot 'Invoke-GPIHub-V117-ModelBakeoff-REV1-REV5-Generated.ps1'
...
if (@($errors).Count -gt 0) {
    ...
    throw "Bakeoff REV5 generated controller parse failed: $text"
}

Write-Host 'V117_BAKEOFF_REV5_CONTROLLER_PATCH=PASS' -ForegroundColor Green
Write-Host "V117_BAKEOFF_REV5_BASE_CONTROLLER=$BaseControllerCommit"
Write-Host "V117_BAKEOFF_REV5_BASE_CONTROLLER_BLOB=$ExpectedBaseControllerBlob"
Write-Host 'V117_BAKEOFF_REV5_AUTHORITY_GUARD_STAGED=PASS' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV5_BUSINESS_CONTEXT_EXPANSION_STAGED=PASS' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV5_PYTEST_CWD=CANDIDATE_ROOT' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV5_ADDITIONAL_COMPILE_ASSERTION=CONFIGURED' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV5_IMPORT_ORIGIN_ASSERTION=CONFIGURED' -ForegroundColor Green
Write-Host 'V117_BAKEOFF_REV5_PRODUCTION_MUTATION=NONE' -ForegroundColor Green
Write-Host "V117_BAKEOFF_REV5_GENERATED_CONTROLLER=$GeneratedController"
